...
let useMockAPI = true; // Always use mock API for now
let connectionIssuesDetected = true;
...
const mockApi = {
  post: async (endpoint, data) => {
    console.log(`Using MOCK API for POST ${endpoint}`, data);
    ...
    if (endpoint === '/auth/login') {
      ...
      // Add to mockDb
      MockAPI.codes.addCustomCode(newCode);
  ...
  delete: async (endpoint) => {
    ...
    if (endpoint.startsWith('/codes/')) {
      // Parse the ID correctly from the endpoint
      const idStr = endpoint.replace('/codes/', '');
      const id = parseInt(idStr);
      console.log('Attempting to delete code with ID:', id, 'from endpoint:', endpoint);
      
      if (isNaN(id) && idStr === 'undefined') {
        console.error('Invalid ID: undefined. Cannot delete code.');
        return { 
          data: { 
            message: 'Error: Cannot delete code with undefined ID' 
          },
          status: 400
        };
      }
      ...
      return MockAPI.codes.deleteAllCodes();
    }
    return { data: { message: 'Operation completed' } };
  }
};
